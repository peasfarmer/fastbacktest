...
type StatisticHandler interface {
	...
	Update(TickerInterface, PortfolioHandler)
...
// Update the complete statistics to a given dataProvider event.
func (s *Statistic) Update(d TickerInterface, p PortfolioHandler) {
	// create new equity point based on current dataProvider timestamp and portfolio value
	e := equityPoint{}
	e.timestamp = d.Time()
	e.equity = p.Value()
...
func (s Statistic) PrintResult() {
	...
	for k, v := range s.Transactions() {
		fmt.Printf("%d. Transaction: %v Action: %v Price: %f Qty: %d\n", k+1, v.Time().Format("2006-01-02 15:04:05.999999999"), v.Direction(), v.Price(), v.Qty())
	}
